...
class Mark(MarkProcess):
    ...
    def make_map(self, file=None, data=None) -> dict:
        """
        从不同源 读取数据生成 map
        :param file: txt
        :param date: excel
        ...
        if file:
            for args in self.file.read_yield(file):
                ...
                num = args["num"]
                insert_args = {
                    "opt": [[int(i["x"]), int(i["y"])] if len(data[headers.index("points")]) != 1 else [int(i["x"]), int(i["y"]), 1] for i in data[headers.index("points")]],
